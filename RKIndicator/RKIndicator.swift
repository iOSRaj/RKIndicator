...
public class RKIndicator: UIView {
    
    private var coverView : UIView?
    private var titleLabel : UILabel?
    private var loadingView : RKIndicatorView?
    ...
    class var sharedInstance: RKIndicator {
        struct Singleton {
            static let instance = RKIndicator(frame: CGRectMake(0, 0, Config().size,Config().size))
        }
    ...
    public class func show(title: String?, animated: Bool, topMargin: Int) {
        let appDelegate:UIWindow = ((UIApplication.sharedApplication().delegate?.window)!)!
        
        let currentWindow : UIWindow = appDelegate
        
        let loader = RKIndicator.sharedInstance
    ...
    private func update() {
        ...
        if (self.loadingView == nil) {
            self.loadingView =  RKIndicatorView(frame: self.frameForSpinner())
    ...
    func orientationChanged() {
        ...
        let height : CGFloat = UIScreen.mainScreen().bounds.size.height
        let width : CGFloat = UIScreen.mainScreen().bounds.size.width
        let center : CGPoint = CGPointMake(width / 2.0, height / 2.0 - CGFloat(0))
        loader.center = center
    }
